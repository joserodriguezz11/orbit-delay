...
juce::AudioProcessorValueTreeState::ParameterLayout createLayout() {
    ...
    for (int i = 0; i < orbit::OrbitEngine::kNumTaps; ++i) {
        ...
        // Version hint = the wave that introduced the parameter; never renumber existing params.
        parameters.push_back(std::make_unique<juce::AudioParameterBool>(
            juce::ParameterID(tapReverseId(i), 2), "Tap " + n + " Reverse", false));
        parameters.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID(tapPitchId(i), 2), "Tap " + n + " Pitch",
    ...
    parameters.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID(kWidthId, 2), "Width",
        juce::NormalisableRange<float>(0.0f, 2.0f), 1.0f));
    parameters.push_back(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID(kPingPongId, 2), "Ping-Pong", false));
    ...
    parameters.push_back(std::make_unique<juce::AudioParameterChoice>(
        juce::ParameterID(kCharacterModeId, 2), "Character", kCharacterChoices, 0));
    ...
    parameters.push_back(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID(kFreezeId, 2), "Freeze", false));
